...
func (r *Repository) CreateEstate(ctx context.Context, input CreateEstateInput) (output CreateEstateOutput, err error) {
	id := uuid.New().String()
	_, err = r.Db.ExecContext(ctx, "INSERT INTO estates (id, length, width) VALUES ($1, $2, $3)", id, input.Length, input.Width)
	if err != nil {
		return
	}
	output.Id = id
...
func (r *Repository) GetEstateById(ctx context.Context, id string) (estate Estate, err error) {
	err = r.Db.QueryRowContext(ctx, "SELECT id, length, width FROM estates WHERE id = $1", id).Scan(&estate.Id, &estate.Length, &estate.Width)
	return
}

func (r *Repository) CreateTree(ctx context.Context, input CreateTreeInput) (output CreateTreeOutput, err error) {
	id := uuid.New().String()
	_, err = r.Db.ExecContext(ctx, "INSERT INTO trees (id, estate_id, x, y, height) VALUES ($1, $2, $3, $4, $5)", id, input.EstateId, input.X, input.Y, input.Height)
	if err != nil {
		return
	}
	output.Id = id
...
func (r *Repository) GetTreesByEstateId(ctx context.Context, input GetTreesByEstateIdInput) (output GetTreesByEstateIdOutput, err error) {
	rows, err := r.Db.QueryContext(ctx, "SELECT id, estate_id, x, y, height FROM trees WHERE estate_id = $1", input.EstateId)
	if err != nil {
		return
	}
	defer rows.Close()
